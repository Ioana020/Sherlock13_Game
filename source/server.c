...
int main(int argc, char *argv[])
{
        	...
        	switch (buffer[0])
        	{
                                ...
                                if (nbClients==4)
				{
					...
					for (int i=0;i<8;i++)
					{
						...
						sendMessageToClient(tcpClients[0].ipAddress,tcpClients[0].port,reply); // Je renvoie chaque case de la ligne de tablesCartes
					}
					...
					for (int i=0;i<8;i++)
					{
						...
						sendMessageToClient(tcpClients[1].ipAddress,tcpClients[1].port,reply); // Je renvoie chaque case de la ligne de tablesCartes
					}
					...
					for (int i=0;i<8;i++)
					{
						...
						sendMessageToClient(tcpClients[2].ipAddress,tcpClients[2].port,reply); // Je renvoie chaque case de la ligne de tablesCartes
					}
					...
					for (int i=0;i<8;i++)
					{
						...
						sendMessageToClient(tcpClients[3].ipAddress,tcpClients[3].port,reply); // Je renvoie chaque case de la ligne de tablesCartes
					}
	...
	else if (fsmServer==1) // j attends les messages 
	{
		switch (buffer[0])
		{
	...
	case 'G':
		{
				// RAJOUTER DU CODE ICI
				// le joueur essaye de deviner une carte directement, Si il a bon, la partie se termine, Sinon, la main passe

    int id, reponse;
    sscanf(buffer, "G %d %d", &id, &reponse);
    
    if (id != joueurCourant) {
        printf("Ce n'est pas le tour du joueur ! Actuel : %d, Reçu : %d\n", joueurCourant, id);
        break;
    }
    
    printf("Le joueur %d a deviné %d (le coupable est %d)\n", id, reponse, deck[12]);
    
    if (reponse == deck[12]) {
        printf("Bonne réponse ! Le joueur %d gagne !\n", id);
        ...
        printf("Mauvaise réponse. Tour suivant.\n");
    ...
    break;
        ...
        case 'O':
					{
				// RAJOUTER DU CODE ICI
				// le joueur essaye de deviner un objet, Si il a bon, la partie se termine, Sinon, la main passe

    int id, objetSel;
    sscanf(buffer, "O %d %d", &id, &objetSel);
    
    if (id != joueurCourant) break;
    
    //
    int total = 0;
    for (int i = 0; i < 4; i++) {
        if (i != id) {
            total += tableCartes[i][objetSel];
            // 
            tableCartes[i][objetSel] = (tableCartes[i][objetSel] > 0) ? 1 : 0;
    ...
    // 
    int culpritCard = deck[12];
    int isCulpritObject = 0;
    
    switch(culpritCard) {
        case 0: isCulpritObject = (objetSel == 7 || objetSel == 2); break;
        case 1: isCulpritObject = (objetSel == 7 || objetSel == 1 || objetSel == 5); break;
        case 2: isCulpritObject = (objetSel == 3 || objetSel == 6 || objetSel == 4); break;
        case 3: isCulpritObject = (objetSel == 3 || objetSel == 2 || objetSel == 4); break;
        case 4: isCulpritObject = (objetSel == 3 || objetSel == 1); break;
        case 5: isCulpritObject = (objetSel == 3 || objetSel == 2); break;
        case 6: isCulpritObject = (objetSel == 3 || objetSel == 0 || objetSel == 6); break;
        case 7: isCulpritObject = (objetSel == 0 || objetSel == 1 || objetSel == 2); break;
        case 8: isCulpritObject = (objetSel == 0 || objetSel == 6 || objetSel == 2); break;
        case 9: isCulpritObject = (objetSel == 0 || objetSel == 1 || objetSel == 4); break;
        case 10: isCulpritObject = (objetSel == 0 || objetSel == 5); break;
        case 11: isCulpritObject = (objetSel == 4 || objetSel == 5); break;
        case 12: isCulpritObject = (objetSel == 7 || objetSel == 1); break;
    }
    
    // 
    if (isCulpritObject && total == 0) {
        sprintf(reply, "W %d", id);
        broadcastMessage(reply);
        fsmServer = 0;
        break;
    ...
    break;
			...
			{
				// RAJOUTER DU CODE ICI
				// le joueur essaye de deviner un symbole, Si il a bon, la partie se termine, Sinon, la main passe
  ...
  int id, joueurSel, objetSel;
    ...
    // Check if asked player has the object
    int hasObject = tableCartes[joueurSel][objetSel] > 0;
    
    // Update table - mark this cell as known (1 if has, 0 if not)
    tableCartes[joueurSel][objetSel] = hasObject ? 1 : 0;
    
    // Broadcast this information to all players
