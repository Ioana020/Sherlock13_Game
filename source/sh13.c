	...
	if (SDL_PollEvent(&event))
	{
		...
		switch (gbuffer[0])
		{
			...
			case 'I':
				...
				sprintf(sendBuffer,"N %d %s",gId,gName);
				sendMessageToServer(gServerIpAddress,gServerPort,sendBuffer);
			...
			case 'D':
			{
				...
				// On envoie ses cartes au serveur
				sprintf(sendBuffer,"C %d %d %d",gId,b[0],b[1]);
				sendMessageToServer(gServerIpAddress,gServerPort,sendBuffer);
			...
			case 'V':
				// RAJOUTER DU CODE ICI
				{int id,ind,val;
				sscanf(gbuffer,"V %d %d %d",&id,&ind,&val);
				printf("id=%d ind=%d val=%d\n",id,ind,val);
				tableCartes[id][ind]=val;
